...
// Executes the `zweb restart` command to
// restart one or more ZWEB Builder
#[derive(Debug, Args)]
#[clap(group(
...
/// Restart one or more ZWEB Builder
pub struct Cmd {
    /// Restart Self-hosted ZWEB Builder
    #[clap(short = 'S', long = "self", action = SetTrue)]
    self_host: bool,

    /// Restart ZWEB Builder on ZWEB Cloud
...
async fn restart_local() -> Result {
    println!("{} Trying to restart the ZWEB Builder...", ui::emoji::BUILD);
    ...
    let stop_builder = _docker.restart_container("zweb_builder", options).await;
    if stop_builder.is_err() {
        println!(
            "{} {} {}",
            ui::emoji::FAIL,
            String::from("Try to restart ZWEB Builder error:"),
    ...
    println!(
        "{} {}",
        ui::emoji::SUCCESS,
        style("Successfully restart the ZWEB Builder.").green(),
    );
